...
public class AuthController : ControllerBase
{
    ...
    [HttpGet("check-user")]
    [AllowAnonymous]
    public async Task<IActionResult> CheckUser([FromQuery] string melliCode)
    {
        if (string.IsNullOrWhiteSpace(melliCode))
            return BadRequest(ApiResponse<object>.Fail("کد ملی وارد نشده است"));

        var phones = await _authService.GetUserPhonesAsync(melliCode);

        return Ok(ApiResponse<object>.Ok(new
        {
            exists = phones.Count > 0,
            hasPhone = phones.Count > 0,
            melliCode
        }));
    }
    ...
    public async Task<IActionResult> ExchangeCode([FromBody] ExchangeCodeRequest request)
    {
        if (string.IsNullOrWhiteSpace(request.Code))
            return BadRequest(ApiResponse<object>.Fail("کد احراز هویت الزامی است"));

        var ssoProvider = ResolveProvider(request.Provider);
        if (ssoProvider == null)
            return BadRequest(ApiResponse<object>.Fail("پروایدر SSO نامعتبر است"));
        ...
        var query = new Microsoft.AspNetCore.Http.QueryCollection(queryParams);

        var result = await ssoProvider.HandleCallbackAsync(query);
        if (!result.IsSuccess || result.UserInfo == null)
            return BadRequest(ApiResponse<object>.Fail(result.Error ?? "خطا در ارتباط با سامانه SSO"));

        if (string.IsNullOrWhiteSpace(result.UserInfo.MelliCode))
            return BadRequest(ApiResponse<object>.Fail("کد ملی در اطلاعات SSO یافت نشد"));

        var tokenResult = await _authService.ProcessMinistrySSOLoginAsync(result.UserInfo);

        return Ok(ApiResponse<ExchangeCodeResponse>.Ok(new ExchangeCodeResponse
        {
